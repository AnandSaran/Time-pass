package com.gregantech.timepass.widget.heart
    ...
    init {
        attrs?.also {
            context.obtainStyledAttributes(it, R.styleable.HeartsView).apply {
                val xMax =
                    getFloat(R.styleable.HeartsView_x_max, HeartsRenderer.DEFAULT_CONFIG.xMax)
                val sizeCoeff = getFloat(
                    R.styleable.HeartsView_size_coeff,
                    HeartsRenderer.DEFAULT_CONFIG.sizeCoeff
                )
                val timeCoeff = getFloat(
                    R.styleable.HeartsView_floating_time_coeff,
